...
import {readBits as naiveReadBits} from './naive-read-bits';
import {readBits as optimizedReadBits} from './read-bits';

describe('Comparison of readBits() implementations:', () => {
    jsc.property(
      'Optimized.readBits(x, y, z) === Naive.readBits(x, y, z)',
      jsc.array(jsc.uint8), jsc.uint8, jsc.nat(12),
      (x, y, z) => optimizedReadBits(x, y, z) === naiveReadBits(x, y, z)
    );
});

describe('Optimized implementation of readBits():', () => {
  let readBits: typeof optimizedReadBits;

  before(() => readBits = optimizedReadBits);
